...
impl MigrationTrait for Migration {
        ...
        manager
            .create_table(
                Table::create()
                    ...
                    .col(
                        ColumnDef::new(Alias::new("date_evenement"))
                            .string()
                            .not_null(),
                    )
                    .col(
                        ColumnDef::new(Alias::new("nb_personnes"))
                            .integer()
                            .not_null(),
                    )
                    ...
                    .col(
                        ColumnDef::new_with_type(
                            Alias::new("statut"),
                            ColumnType::Enum {
                                name: Alias::new("StatutDevis").into_iden(),
                                variants: vec![
                                    Alias::new("en_attente").into_iden(),
                                    Alias::new("en_cours").into_iden(),
                                    Alias::new("accepte").into_iden(),
                                    Alias::new("refuse").into_iden(),
                                ],
                            },
                        )
                        .not_null(),
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(Table::drop().table(Alias::new("devis_traiteur")).to_owned())
            .await?;
        manager
            .get_connection()
            .execute_unprepared("DROP TYPE IF EXISTS StatutDevis")
            .await?;
