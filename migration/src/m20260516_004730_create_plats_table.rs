...
async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.get_connection().execute_unprepared(
            "DO $$ BEGIN CREATE TYPE TypePlat AS ENUM ('entree', 'specialite', 'viande', 'poisson', 'plat', 'dessert'); EXCEPTION WHEN duplicate_object THEN NULL; END $$"
        ).await?;
        ...
        manager
            .create_table(
                Table::create()
                    ...
                    .col(ColumnDef::new_with_type(Alias::new("type_plat"), ColumnType::Enum { name: Alias::new("TypePlat").into_iden(), variants: vec![Alias::new("entree").into_iden(), Alias::new("specialite").into_iden(), Alias::new("viande").into_iden(), Alias::new("poisson").into_iden(), Alias::new("plat").into_iden(), Alias::new("dessert").into_iden()] }).not_null())
