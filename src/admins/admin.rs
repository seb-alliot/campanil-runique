...
pub fn admin_register() -> AdminRegistry {
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            const SORT_COLS: &[&str] = &["id", "jour", "ouverture_midi", "fermeture_midi", "ouverture_soir", "fermeture_soir", "ferme"];
        ...
        Box::pin(async move {
            ...
            for val in values {
                ...
                let existing_id: Option<String> = horaire::Entity::find()
                    .filter(Expr::col(Alias::new("jour")).cast_as(Alias::new("TEXT")).eq(val.to_string()))
                    .one(&*db).await?
                    .map(|m| m.id.to_string());
                if let Some(id) = existing_id {
                    let id = id.parse::<Pk>().map_err(|_| DbErr::Custom("invalid id".to_string()))?;
                    horaire::admin_from_form(&row, Some(id)).update(&*db).await?;
                } else {
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let mut vals_ferme: Vec<String> = rows_ferme.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_ferme.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            const SORT_COLS: &[&str] = &["id", "nom", "email", "date_evenement", "nb_personnes", "prix_total", "remise_appliquee", "statut", "created_at"];
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("nom", "Nom"), ("email", "Email"), ("date_evenement", "Date événement"), ("nb_personnes", "Personnes"), ("prix_total", "Estimation"), ("remise_appliquee", "Remise"), ("statut", "Statut"), ("created_at", "Reçu le")]).list_filter(vec![("statut", "Statut", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let mut vals_statut: Vec<String> = rows_statut.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_statut.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("raison", "Raison"), ("titre", "Sujet"), ("email", "Email"), ("created_at", "Reçu le")]).list_filter(vec![("raison", "Raison", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let count_stmt_raison = Query::select().expr(Expr::cust("COUNT(DISTINCT raison)")).from(Alias::new(contact::Entity.table_name())).and_where(Expr::col(Alias::new("raison")).is_not_null()).to_owned();
            let count_row_raison = match db.query_one(&count_stmt_raison).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `contacts.raison`: column not found in DB — {}", e); None } };
            let total_raison = count_row_raison.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_raison = Query::select().distinct().expr(Expr::cust("CAST(raison AS TEXT)")).from(Alias::new(contact::Entity.table_name())).and_where(Expr::col(Alias::new("raison")).is_not_null()).limit(page_size_raison).offset(cur_page_raison * page_size_raison).to_owned();
            let rows_raison = match db.query_all(&stmt_raison).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `contacts.raison`: column not found in DB — {}", e); vec![] } };
            let mut vals_raison: Vec<String> = rows_raison.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_raison.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let mut vals_type_garniture: Vec<String> = rows_type_garniture.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_type_garniture.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
            ...
            let mut vals_disponible: Vec<String> = rows_disponible.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_disponible.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let meta = meta.fk_display(vec![("garniture_id".to_string(), "garnitures".to_string(), "libelle".to_string())]);
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
    ...
    let create_fn: CreateFn = Arc::new(|db: ADb, data: StrMap| {
        Box::pin(async move {
            ...
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_allergenes__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
        ...
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    ...
                    let stmt = Query::select().expr(Expr::cust("CAST(allergene_id AS TEXT)")).from(Alias::new("entree_allergene")).and_where(Expr::col(Alias::new("entree_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let mut vals_usage: Vec<String> = rows_usage.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_usage.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(allergene_id AS TEXT)")).from(Alias::new("dessert_allergene")).and_where(Expr::col(Alias::new("dessert_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
                    field_name: "allergenes".to_string(),
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            const SORT_COLS: &[&str] = &["id", "titre", "type_plat", "usage", "prix", "disponible", "est_viande"];
    ...
    let create_fn: CreateFn = Arc::new(|db: ADb, data: StrMap| {
        Box::pin(async move {
            ...
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_garnitures__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                    let sql = format!("INSERT INTO plat_garnitures (plat_id, garniture_id) VALUES ({}, {}) ON CONFLICT DO NOTHING", inserted_id, target_id);
                    let _ = db.execute_unprepared(&sql).await;
                }
            }
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_supplements__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                    let sql = format!("INSERT INTO plat_supplements (plat_id, supplement_id) VALUES ({}, {}) ON CONFLICT DO NOTHING", inserted_id, target_id);
                    let _ = db.execute_unprepared(&sql).await;
                }
            }
            Ok(())
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            ...
            let _ = db.execute_unprepared(&format!("DELETE FROM plat_supplements WHERE plat_id = {}", id_str)).await;
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_supplements__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                    let sql = format!("INSERT INTO plat_supplements (plat_id, supplement_id) VALUES ({}, {})", id_str, target_id);
                    let _ = db.execute_unprepared(&sql).await;
                }
            }
            Ok(())
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(allergene_id AS TEXT)")).from(Alias::new("plat_allergene")).and_where(Expr::col(Alias::new("plat_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
                    field_name: "allergenes".to_string(),
            ...
            }
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(garniture_id AS TEXT)")).from(Alias::new("plat_garnitures")).and_where(Expr::col(Alias::new("plat_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
            ...
            {
                let rows = crate::entities::supplement::Entity::find().all(&*db).await.unwrap_or_default();
                let choices: Vec<(String, String)> = rows.iter().map(|r| {
                    let v = serde_json::to_value(r).unwrap_or_default();
                    let id = v.get("id").map(|i| i.to_string().trim_matches('"').to_string()).unwrap_or_default();
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(supplement_id AS TEXT)")).from(Alias::new("plat_supplements")).and_where(Expr::col(Alias::new("plat_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
                    field_name: "supplements".to_string(),
                    label: "Suppléments".to_string(),
                    choices,
                    selected,
                });
            }
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let mut vals_type_plat: Vec<String> = rows_type_plat.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_type_plat.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
            ...
            let mut vals_est_viande: Vec<String> = rows_est_viande.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_est_viande.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let create_fn: CreateFn = Arc::new(|db: ADb, data: StrMap| {
        Box::pin(async move {
            ...
            let inserted_id = result.id.to_string();
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_entrees__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                    let sql = format!("INSERT INTO menu_entrees (menu_id, entree_id) VALUES ({}, {}) ON CONFLICT DO NOTHING", inserted_id, target_id);
                    let _ = db.execute_unprepared(&sql).await;
                }
            }
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_plats__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                    let sql = format!("INSERT INTO menu_plats (menu_id, plat_id) VALUES ({}, {}) ON CONFLICT DO NOTHING", inserted_id, target_id);
                    let _ = db.execute_unprepared(&sql).await;
                }
            }
            for key in data.keys() {
                if let Some(target_id) = key.strip_prefix("m2m_desserts__") && !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(entree_id AS TEXT)")).from(Alias::new("menu_entrees")).and_where(Expr::col(Alias::new("menu_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
            ...
            }
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(plat_id AS TEXT)")).from(Alias::new("menu_plats")).and_where(Expr::col(Alias::new("menu_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
            ...
            }
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(dessert_id AS TEXT)")).from(Alias::new("menu_desserts")).and_where(Expr::col(Alias::new("menu_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let mut vals_type_menu: Vec<String> = rows_type_menu.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_type_menu.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            const SORT_COLS: &[&str] = &["id", "titre", "theme", "regime", "prix_par_personne", "nb_personnes_min", "remise_groupe", "remise_groupe_min", "stock", "actif"];
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr, ExprTrait};
                    let stmt = Query::select().expr(Expr::cust("CAST(plat_id AS TEXT)")).from(Alias::new("menu_traiteur_plats")).and_where(Expr::col(Alias::new("menu_traiteur_id")).cast_as(Alias::new("TEXT")).eq(oid.clone())).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
                    field_name: "plats".to_string(),
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("titre", "Titre"), ("theme", "Thème"), ("regime", "Régime"), ("prix_par_personne", "Prix / pers."), ("nb_personnes_min", "Personnes min"), ("remise_groupe", "Remise groupe (%)"), ("remise_groupe_min", "Remise à partir de (pers.)"), ("stock", "Stock"), ("actif", "Actif")]).list_filter(vec![("theme", "Thème", 10u64), ("regime", "Régime", 10u64), ("actif", "Actif", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let mut vals_regime: Vec<String> = rows_regime.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_regime.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
            ...
            let mut vals_actif: Vec<String> = rows_actif.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_actif.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let mut vals_type_boisson: Vec<String> = rows_type_boisson.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_type_boisson.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            const SORT_COLS: &[&str] = &["id", "numero", "user_id", "type_retrait", "statut", "pret_materiel", "mode_paiement", "prix_total", "created_at"];
    ...
    let edit_form_builder: FormBuilder = Arc::new(|_db: ADb, _vec: Vec<std::string::String>, data: StrMap, tera: ATera, csrf: String, method: Method| {
        Box::pin(async move {
            let form = crate::formulaire::CommandeAdminEditForm::build_with_data(&data, tera, &csrf, method).await;
            Box::new(CommandeEditFormDynWrapper(form)) as Box<dyn DynForm>
        })
    });

    let meta = meta.display(DisplayConfig::new().columns_include(vec![("numero", "N°"), ("user_id", "Client"), ("type_retrait", "Type"), ("statut", "Statut"), ("pret_materiel", "Matériel rendu"), ("mode_paiement", "Paiement"), ("prix_total", "Total"), ("created_at", "Date")]).list_filter(vec![("type_retrait", "Type", 10u64), ("statut", "Statut", 10u64), ("mode_paiement", "Paiement", 10u64)]));
    let meta = meta.fk_display(vec![("user_id".to_string(), "eihwaz_users".to_string(), "username".to_string())]);
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let mut vals_type_retrait: Vec<String> = rows_type_retrait.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_type_retrait.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
            ...
            let mut vals_mode_paiement: Vec<String> = rows_mode_paiement.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_mode_paiement.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => y.cmp(&x), _ => b.cmp(a) });
    ...
    let meta = meta.fk_display(vec![("commande_id".to_string(), "commandes".to_string(), "numero".to_string())]);
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
    ...
    let meta = meta.fk_display(vec![("plat_id".to_string(), "plats".to_string(), "titre".to_string())]);
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            const SORT_COLS: &[&str] = &["id", "nom", "adresse", "telephone", "email", "penalite_materiel", "latitude", "longitude"];
            const FILTER_COLS: &[&str] = &[];
            if let Some(ref col) = params.sort_by && SORT_COLS.contains(&col.as_str()) {
                let order = if params.sort_dir == SortDir::Desc { Order::Desc } else { Order::Asc };
                query = query.order_by(Expr::col(Alias::new(col.as_str())), order);
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("nom", "Nom"), ("adresse", "Adresse"), ("telephone", "Téléphone"), ("email", "Email"), ("penalite_materiel", "Pénalité matériel (€)"), ("latitude", "Latitude"), ("longitude", "Longitude")]));
    registry.register(
        ResourceEntry::new(meta, form_builder)
            .with_list_fn(list_fn)
            .with_get_fn(get_fn)
