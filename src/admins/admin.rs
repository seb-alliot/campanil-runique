...
pub fn admin_register() -> AdminRegistry {
    ...
    let meta = meta.display(
        DisplayConfig::new()
            ...
            .list_filter(vec![("raison", "Raison", 10u64)]),
    );
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::sea_query::{Alias, Expr, Query};
            ...
            let count_stmt_raison = Query::select()
                ...
                .to_owned();
            let count_row_raison = match db.query_one(&count_stmt_raison).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `contacts.raison`: column not found in DB — {}",
                        e
                    );
                    None
                }
            };
            let total_raison = count_row_raison
                .and_then(|r| r.try_get_by_index::<i64>(0).ok())
                .unwrap_or(0) as u64;
            let stmt_raison = Query::select()
                .distinct()
                .expr(Expr::cust("CAST(raison AS TEXT)"))
                .from(Alias::new(contact::Entity.table_name()))
                .and_where(Expr::col(Alias::new("raison")).is_not_null())
                .limit(page_size_raison)
                .offset(cur_page_raison * page_size_raison)
                .to_owned();
            let rows_raison = match db.query_all(&stmt_raison).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `contacts.raison`: column not found in DB — {}",
            ...
            let mut vals_raison: Vec<String> = rows_raison
                .iter()
                .filter_map(|r| r.try_get_by_index::<String>(0).ok())
                .collect();
        ...
        Box::pin(async move {
            ...
            let _ = db
                .execute_unprepared(&format!(
                    "DELETE FROM entree_allergene WHERE entree_id = {}",
                    id_str
                ))
                .await;
            for key in data.keys() {
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_stmt_usage = Query::select()
                ...
                .to_owned();
            let count_row_usage = match db.query_one(&count_stmt_usage).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `entrees.usage`: column not found in DB — {}",
                        e
                    );
                    None
                }
            };
            let total_usage = count_row_usage
                .and_then(|r| r.try_get_by_index::<i64>(0).ok())
                .unwrap_or(0) as u64;
            let stmt_usage = Query::select()
                .distinct()
                .expr(Expr::cust("CAST(usage AS TEXT)"))
                .from(Alias::new(entree::Entity.table_name()))
                .and_where(Expr::col(Alias::new("usage")).is_not_null())
                .limit(page_size_usage)
                .offset(cur_page_usage * page_size_usage)
                .to_owned();
            let rows_usage = match db.query_all(&stmt_usage).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `entrees.usage`: column not found in DB — {}",
            ...
            let mut vals_usage: Vec<String> = rows_usage
                .iter()
                .filter_map(|r| r.try_get_by_index::<String>(0).ok())
                .collect();
            ...
            let count_row_disponible = match db.query_one(&count_stmt_disponible).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `entrees.disponible`: column not found in DB — {}",
                        e
                    );
                    None
                }
    ...
    let delete_fn: DeleteFn = Arc::new(|db: ADb, id: String| {
        Box::pin(async move {
            let id = id
                .parse::<Pk>()
                .map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            dessert::Entity::delete_by_id(id)
                .exec(&*db)
                .await
                .map(|_| ())
        })
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            ...
            let _ = db
                .execute_unprepared(&format!(
                    "DELETE FROM dessert_allergene WHERE dessert_id = {}",
                    id_str
                ))
                .await;
            for key in data.keys() {
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_row_usage = match db.query_one(&count_stmt_usage).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `desserts.usage`: column not found in DB — {}",
                        e
                    );
                    None
                }
            ...
            let count_row_disponible = match db.query_one(&count_stmt_disponible).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `desserts.disponible`: column not found in DB — {}",
                        e
                    );
                    None
                }
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            {
                let rows = crate::entities::supplement::Entity::find()
                    .all(&*db)
                    .await
                    .unwrap_or_default();
                let choices: Vec<(String, String)> = rows
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_row_usage = match db.query_one(&count_stmt_usage).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `plats.usage`: column not found in DB — {}",
                        e
                    );
                    None
                }
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                let rows = crate::entities::entree::Entity::find()
                    .all(&*db)
                    .await
                    .unwrap_or_default();
                let choices: Vec<(String, String)> = rows
            ...
            {
                let rows = crate::entities::dessert::Entity::find()
                    .all(&*db)
                    .await
                    .unwrap_or_default();
                let choices: Vec<(String, String)> = rows
    ...
    let meta = meta.display(
        DisplayConfig::new()
            ...
            .list_filter(vec![("type_menu", "Type", 10u64)]),
    );
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::sea_query::{Alias, Expr, Query};
            ...
            let count_stmt_type_menu = Query::select()
                ...
                .to_owned();
            let count_row_type_menu = match db.query_one(&count_stmt_type_menu).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `menus.type_menu`: column not found in DB — {}",
                        e
                    );
                    None
                }
            };
            let total_type_menu = count_row_type_menu
                .and_then(|r| r.try_get_by_index::<i64>(0).ok())
                .unwrap_or(0) as u64;
            let stmt_type_menu = Query::select()
                .distinct()
                .expr(Expr::cust("CAST(type_menu AS TEXT)"))
                .from(Alias::new(menu::Entity.table_name()))
                .and_where(Expr::col(Alias::new("type_menu")).is_not_null())
                .limit(page_size_type_menu)
                .offset(cur_page_type_menu * page_size_type_menu)
                .to_owned();
            let rows_type_menu = match db.query_all(&stmt_type_menu).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `menus.type_menu`: column not found in DB — {}",
            ...
            let mut vals_type_menu: Vec<String> = rows_type_menu
                .iter()
                .filter_map(|r| r.try_get_by_index::<String>(0).ok())
                .collect();
    ...
    let form_builder: FormBuilder = Arc::new(
        |_db: ADb,
         ...
         method: Method| {
            Box::pin(async move {
                let form =
                    menu_traiteur::AdminForm::build_with_data(&data, tera, &csrf, method).await;
    ...
    let delete_fn: DeleteFn = Arc::new(|db: ADb, id: String| {
        Box::pin(async move {
            let id = id
                .parse::<Pk>()
                .map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            menu_traiteur::Entity::delete_by_id(id)
                .exec(&*db)
                .await
                .map(|_| ())
        })
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_row_theme = match db.query_one(&count_stmt_theme).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `menus_traiteur.theme`: column not found in DB — {}",
                        e
                    );
                    None
                }
            ...
            let count_row_regime = match db.query_one(&count_stmt_regime).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `menus_traiteur.regime`: column not found in DB — {}",
                        e
                    );
                    None
                }
            ...
            let count_row_actif = match db.query_one(&count_stmt_actif).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `menus_traiteur.actif`: column not found in DB — {}",
                        e
                    );
                    None
                }
