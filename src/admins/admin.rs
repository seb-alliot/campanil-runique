...
pub fn admin_register() -> AdminRegistry {
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "libelle", escaped)));
    ...
    let delete_fn: DeleteFn = Arc::new(|db: ADb, id: String| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            regime::Entity::delete_by_id(id).exec(&*db).await.map(|_| ())
        })
    });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "jour", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "ouverture_midi", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "fermeture_midi", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "ouverture_soir", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "fermeture_soir", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "ferme", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            let rows: Vec<serde_json::Value> = db_rows.into_iter()
    ...
    let form_builder: FormBuilder = Arc::new(|db: ADb, _vec: Vec<std::string::String>, data: StrMap, tera: ATera, csrf: String, method: Method| {
        Box::pin(async move {
            let mut form = devis_traiteur::AdminForm::build_with_data(&data, tera, &csrf, method).await;
            {
                use sea_orm::ConnectionTrait;
                let _fk_opt_stmt_menu_id = sea_orm::sea_query::Query::select().expr(sea_orm::sea_query::Expr::cust("CAST(id AS TEXT)")).expr(sea_orm::sea_query::Expr::cust("titre")).from(sea_orm::sea_query::Alias::new("menus")).to_owned();
                let _fk_opt_choices_menu_id: Vec<(String, String)> = db.query_all(&_fk_opt_stmt_menu_id).await.unwrap_or_default().iter().filter_map(|row| { let id = row.try_get_by_index::<String>(0).ok()?; let lbl = row.try_get_by_index::<String>(1).ok()?; Some((id, lbl)) }).collect();
                form.get_form_mut().field_choices("menu_id", "Menu", _fk_opt_choices_menu_id);
            }
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "nom", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "email", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "date_evenement", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "nb_personnes", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "statut", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "created_at", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            ...
            {
                use sea_orm::ConnectionTrait;
                let fk_ids: Vec<String> = rows.iter().filter_map(|r| r.get("menu_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string)))).collect::<std::collections::HashSet<String>>().into_iter().collect();
                if !fk_ids.is_empty() {
                    let ids_csv = fk_ids.iter().map(|s| format!("'{}'", s.replace('\'', "''"))).collect::<Vec<_>>().join(",");
                    let _fk_stmt_menu_id = sea_orm::sea_query::Query::select().expr(sea_orm::sea_query::Expr::cust("CAST(id AS TEXT)")).expr(sea_orm::sea_query::Expr::cust("titre")).from(sea_orm::sea_query::Alias::new("menus")).and_where(sea_orm::sea_query::Expr::cust(format!("CAST(id AS TEXT) IN ({})", ids_csv))).to_owned();
                    let label_map_menu_id: std::collections::HashMap<String, String> = db.query_all(&_fk_stmt_menu_id).await.unwrap_or_default().iter().filter_map(|row| { let id = row.try_get_by_index::<String>(0).ok()?; let label = row.try_get_by_index::<String>(1).ok()?; Some((id, label)) }).collect();
                    for row in &mut rows {
                        if let Some(key) = row.get("menu_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string))) {
                            if let Some(label) = label_map_menu_id.get(&key) {
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            devis_traiteur::admin_from_form(&data, Some(id.into()))
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            devis_traiteur::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });

    let meta = meta.display(DisplayConfig::new().columns_include(vec![("nom", "Nom"), ("email", "Email"), ("menu_id", "Menu"), ("date_evenement", "Date événement"), ("nb_personnes", "Personnes"), ("statut", "Statut"), ("created_at", "Reçu le")]).list_filter(vec![("statut", "Statut", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "titre", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "email", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "created_at", escaped)));
                query = query.filter(search_cond);
            }
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "libelle", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "type_garniture", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "disponible", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            let rows: Vec<serde_json::Value> = db_rows.into_iter()
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            garniture::admin_from_form(&data, Some(id.into()))
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            garniture::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "titre", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "type_plat", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "prix", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "disponible", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "est_viande", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "avec_legumes", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            let rows: Vec<serde_json::Value> = db_rows.into_iter()
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            ...
            plat::admin_from_form(&data, Some(id.into())).update(&*db).await?;
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("titre", "Titre"), ("type_plat", "Type"), ("prix", "Prix"), ("disponible", "Disponible"), ("est_viande", "Viande"), ("avec_legumes", "Légumes inclus")]).list_filter(vec![("type_plat", "Type de plat", 10u64), ("disponible", "Disponibilité", 10u64), ("est_viande", "Viande", 10u64), ("avec_legumes", "Légumes", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let count_stmt_avec_legumes = Query::select().expr(Expr::cust("COUNT(DISTINCT avec_legumes)")).from(Alias::new(plat::Entity.table_name())).and_where(Expr::col(Alias::new("avec_legumes")).is_not_null()).to_owned();
            let count_row_avec_legumes = match db.query_one(&count_stmt_avec_legumes).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `plats.avec_legumes`: column not found in DB — {}", e); None } };
            let total_avec_legumes = count_row_avec_legumes.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_avec_legumes = Query::select().distinct().expr(Expr::cust("CAST(avec_legumes AS TEXT)")).from(Alias::new(plat::Entity.table_name())).and_where(Expr::col(Alias::new("avec_legumes")).is_not_null()).limit(page_size_avec_legumes).offset(cur_page_avec_legumes * page_size_avec_legumes).to_owned();
            let rows_avec_legumes = match db.query_all(&stmt_avec_legumes).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `plats.avec_legumes`: column not found in DB — {}", e); vec![] } };
            let mut vals_avec_legumes: Vec<String> = rows_avec_legumes.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_avec_legumes.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
            result.insert("avec_legumes".to_string(), (vals_avec_legumes, total_avec_legumes));
            Ok(result)
        })
    });
    ...
    let form_builder: FormBuilder = Arc::new(|db: ADb, _vec: Vec<std::string::String>, data: StrMap, tera: ATera, csrf: String, method: Method| {
        Box::pin(async move {
            let mut form = menu::AdminForm::build_with_data(&data, tera, &csrf, method).await;
            {
                use sea_orm::ConnectionTrait;
                let _fk_opt_stmt_theme_id = sea_orm::sea_query::Query::select().expr(sea_orm::sea_query::Expr::cust("CAST(id AS TEXT)")).expr(sea_orm::sea_query::Expr::cust("libelle")).from(sea_orm::sea_query::Alias::new("themes")).to_owned();
                let _fk_opt_choices_theme_id: Vec<(String, String)> = db.query_all(&_fk_opt_stmt_theme_id).await.unwrap_or_default().iter().filter_map(|row| { let id = row.try_get_by_index::<String>(0).ok()?; let lbl = row.try_get_by_index::<String>(1).ok()?; Some((id, lbl)) }).collect();
                form.get_form_mut().field_choices("theme_id", "Thème", _fk_opt_choices_theme_id);
            }
            {
                use sea_orm::ConnectionTrait;
                let _fk_opt_stmt_regime_id = sea_orm::sea_query::Query::select().expr(sea_orm::sea_query::Expr::cust("CAST(id AS TEXT)")).expr(sea_orm::sea_query::Expr::cust("libelle")).from(sea_orm::sea_query::Alias::new("regimes")).to_owned();
                let _fk_opt_choices_regime_id: Vec<(String, String)> = db.query_all(&_fk_opt_stmt_regime_id).await.unwrap_or_default().iter().filter_map(|row| { let id = row.try_get_by_index::<String>(0).ok()?; let lbl = row.try_get_by_index::<String>(1).ok()?; Some((id, lbl)) }).collect();
                form.get_form_mut().field_choices("regime_id", "Régime", _fk_opt_choices_regime_id);
            }
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "titre", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "prix_par_personne", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "nb_personnes_min", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "actif", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            let mut rows: Vec<serde_json::Value> = db_rows.into_iter()
            ...
            {
                use sea_orm::ConnectionTrait;
                let fk_ids: Vec<String> = rows.iter().filter_map(|r| r.get("theme_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string)))).collect::<std::collections::HashSet<String>>().into_iter().collect();
                if !fk_ids.is_empty() {
                    let ids_csv = fk_ids.iter().map(|s| format!("'{}'", s.replace('\'', "''"))).collect::<Vec<_>>().join(",");
                    let _fk_stmt_theme_id = sea_orm::sea_query::Query::select().expr(sea_orm::sea_query::Expr::cust("CAST(id AS TEXT)")).expr(sea_orm::sea_query::Expr::cust("libelle")).from(sea_orm::sea_query::Alias::new("themes")).and_where(sea_orm::sea_query::Expr::cust(format!("CAST(id AS TEXT) IN ({})", ids_csv))).to_owned();
                    let label_map_theme_id: std::collections::HashMap<String, String> = db.query_all(&_fk_stmt_theme_id).await.unwrap_or_default().iter().filter_map(|row| { let id = row.try_get_by_index::<String>(0).ok()?; let label = row.try_get_by_index::<String>(1).ok()?; Some((id, label)) }).collect();
                    for row in &mut rows {
                        if let Some(key) = row.get("theme_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string))) {
                            if let Some(label) = label_map_theme_id.get(&key) {
                                row["theme_id"] = serde_json::Value::String(label.clone());
                            }
                        }
                    }
                }
            ...
            {
                use sea_orm::ConnectionTrait;
                let fk_ids: Vec<String> = rows.iter().filter_map(|r| r.get("regime_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string)))).collect::<std::collections::HashSet<String>>().into_iter().collect();
                if !fk_ids.is_empty() {
                    let ids_csv = fk_ids.iter().map(|s| format!("'{}'", s.replace('\'', "''"))).collect::<Vec<_>>().join(",");
                    let _fk_stmt_regime_id = sea_orm::sea_query::Query::select().expr(sea_orm::sea_query::Expr::cust("CAST(id AS TEXT)")).expr(sea_orm::sea_query::Expr::cust("libelle")).from(sea_orm::sea_query::Alias::new("regimes")).and_where(sea_orm::sea_query::Expr::cust(format!("CAST(id AS TEXT) IN ({})", ids_csv))).to_owned();
                    let label_map_regime_id: std::collections::HashMap<String, String> = db.query_all(&_fk_stmt_regime_id).await.unwrap_or_default().iter().filter_map(|row| { let id = row.try_get_by_index::<String>(0).ok()?; let label = row.try_get_by_index::<String>(1).ok()?; Some((id, label)) }).collect();
                    for row in &mut rows {
                        if let Some(key) = row.get("regime_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string))) {
                            if let Some(label) = label_map_regime_id.get(&key) {
                                row["regime_id"] = serde_json::Value::String(label.clone());
                            }
                        }
                    }
                }
        ...
        Box::pin(async move {
            ...
            for (key, _) in &data {
                if let Some(target_id) = key.strip_prefix("m2m_plats__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let sql = format!("INSERT INTO menu_plat (menu_id, plat_id) VALUES ({}, {}) ON CONFLICT DO NOTHING", inserted_id, target_id);
                        let _ = db.execute_unprepared(&sql).await;
                    }
                }
            }
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            ...
            use sea_orm::ConnectionTrait;
            let id_str = id.to_string();
            menu::admin_from_form(&data, Some(id.into())).update(&*db).await?;
            let _ = db.execute_unprepared(&format!("DELETE FROM menu_plat WHERE menu_id = {}", id_str)).await;
            for (key, _) in &data {
                if let Some(target_id) = key.strip_prefix("m2m_plats__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let sql = format!("INSERT INTO menu_plat (menu_id, plat_id) VALUES ({}, {})", id_str, target_id);
                        let _ = db.execute_unprepared(&sql).await;
                    }
                }
            }
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let mut fields: Vec<M2mFieldOptions> = Vec::new();
            {
                ...
                let selected = if let Some(ref oid) = object_id {
                    use sea_orm::sea_query::{Query, Alias, Expr};
                    let stmt = Query::select().expr(Expr::cust("CAST(plat_id AS TEXT)")).from(Alias::new("menu_plat")).and_where(Expr::cust(format!("CAST(menu_id AS TEXT) = '{}'", oid))).to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("titre", "Titre"), ("theme_id", "Thème"), ("regime_id", "Régime"), ("prix_par_personne", "Prix/pers."), ("nb_personnes_min", "Min. pers."), ("actif", "Actif")]).list_filter(vec![("actif", "Actif", 10u64), ("theme_id", "Thème", 10u64), ("regime_id", "Régime", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let count_stmt_actif = Query::select().expr(Expr::cust("COUNT(DISTINCT actif)")).from(Alias::new(menu::Entity.table_name())).and_where(Expr::col(Alias::new("actif")).is_not_null()).to_owned();
            let count_row_actif = match db.query_one(&count_stmt_actif).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus.actif`: column not found in DB — {}", e); None } };
            let total_actif = count_row_actif.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_actif = Query::select().distinct().expr(Expr::cust("CAST(actif AS TEXT)")).from(Alias::new(menu::Entity.table_name())).and_where(Expr::col(Alias::new("actif")).is_not_null()).limit(page_size_actif).offset(cur_page_actif * page_size_actif).to_owned();
            let rows_actif = match db.query_all(&stmt_actif).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus.actif`: column not found in DB — {}", e); vec![] } };
            ...
            let count_stmt_theme_id = Query::select().expr(Expr::cust("COUNT(DISTINCT theme_id)")).from(Alias::new(menu::Entity.table_name())).and_where(Expr::col(Alias::new("theme_id")).is_not_null()).to_owned();
            let count_row_theme_id = match db.query_one(&count_stmt_theme_id).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus.theme_id`: column not found in DB — {}", e); None } };
            let total_theme_id = count_row_theme_id.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_theme_id = Query::select().distinct().expr(Expr::cust("CAST(theme_id AS TEXT)")).from(Alias::new(menu::Entity.table_name())).and_where(Expr::col(Alias::new("theme_id")).is_not_null()).limit(page_size_theme_id).offset(cur_page_theme_id * page_size_theme_id).to_owned();
            let rows_theme_id = match db.query_all(&stmt_theme_id).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus.theme_id`: column not found in DB — {}", e); vec![] } };
            let mut vals_theme_id: Vec<String> = rows_theme_id.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_theme_id.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
            ...
            let count_stmt_regime_id = Query::select().expr(Expr::cust("COUNT(DISTINCT regime_id)")).from(Alias::new(menu::Entity.table_name())).and_where(Expr::col(Alias::new("regime_id")).is_not_null()).to_owned();
            let count_row_regime_id = match db.query_one(&count_stmt_regime_id).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus.regime_id`: column not found in DB — {}", e); None } };
            let total_regime_id = count_row_regime_id.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_regime_id = Query::select().distinct().expr(Expr::cust("CAST(regime_id AS TEXT)")).from(Alias::new(menu::Entity.table_name())).and_where(Expr::col(Alias::new("regime_id")).is_not_null()).limit(page_size_regime_id).offset(cur_page_regime_id * page_size_regime_id).to_owned();
            let rows_regime_id = match db.query_all(&stmt_regime_id).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus.regime_id`: column not found in DB — {}", e); vec![] } };
            let mut vals_regime_id: Vec<String> = rows_regime_id.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_regime_id.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
    ...
    let delete_fn: DeleteFn = Arc::new(|db: ADb, id: String| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            menu_resto::Entity::delete_by_id(id).exec(&*db).await.map(|_| ())
        })
    });
    ...
    let create_fn: CreateFn = Arc::new(|db: ADb, data: StrMap| {
        Box::pin(async move {
            use sea_orm::ConnectionTrait;
            let result = menu_resto::admin_from_form(&data, None).insert(&*db).await?;
            let inserted_id = result.id.to_string();
            for (key, _) in &data {
                if let Some(target_id) = key.strip_prefix("m2m_entrees__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let _ = db.execute_unprepared(&format!("INSERT INTO menu_resto_plat (menu_id, plat_id, cours) VALUES ({}, {}, 'entree')", inserted_id, target_id)).await;
                    }
                } else if let Some(target_id) = key.strip_prefix("m2m_plats_menu__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let _ = db.execute_unprepared(&format!("INSERT INTO menu_resto_plat (menu_id, plat_id, cours) VALUES ({}, {}, 'plat')", inserted_id, target_id)).await;
                    }
                } else if let Some(target_id) = key.strip_prefix("m2m_desserts__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let _ = db.execute_unprepared(&format!("INSERT INTO menu_resto_plat (menu_id, plat_id, cours) VALUES ({}, {}, 'dessert')", inserted_id, target_id)).await;
                    }
                }
            }
            Ok(())
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            ...
            use sea_orm::ConnectionTrait;
            let id_str = id.to_string();
            menu_resto::admin_from_form(&data, Some(id.into())).update(&*db).await?;
            let _ = db.execute_unprepared(&format!("DELETE FROM menu_resto_plat WHERE menu_id = {} AND cours = 'entree'", id_str)).await;
            let _ = db.execute_unprepared(&format!("DELETE FROM menu_resto_plat WHERE menu_id = {} AND cours = 'plat'", id_str)).await;
            let _ = db.execute_unprepared(&format!("DELETE FROM menu_resto_plat WHERE menu_id = {} AND cours = 'dessert'", id_str)).await;
            for (key, _) in &data {
                if let Some(target_id) = key.strip_prefix("m2m_entrees__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let _ = db.execute_unprepared(&format!("INSERT INTO menu_resto_plat (menu_id, plat_id, cours) VALUES ({}, {}, 'entree')", id_str, target_id)).await;
                    }
                } else if let Some(target_id) = key.strip_prefix("m2m_plats_menu__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let _ = db.execute_unprepared(&format!("INSERT INTO menu_resto_plat (menu_id, plat_id, cours) VALUES ({}, {}, 'plat')", id_str, target_id)).await;
                    }
                } else if let Some(target_id) = key.strip_prefix("m2m_desserts__") {
                    if !target_id.is_empty() && target_id.chars().all(|c| c.is_ascii_alphanumeric() || c == '-') {
                        let _ = db.execute_unprepared(&format!("INSERT INTO menu_resto_plat (menu_id, plat_id, cours) VALUES ({}, {}, 'dessert')", id_str, target_id)).await;
                    }
                }
            }
            Ok(())
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            menu_resto::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let m2m_loader: M2mLoaderFn = Arc::new(|db: ADb, object_id: Option<String>| {
        Box::pin(async move {
            ...
            let plat_rows_for = |type_filter: &str| {
                Query::select()
                    .expr(Expr::cust("CAST(id AS TEXT)"))
                    .expr(Expr::cust("COALESCE(NULLIF(label, ''), titre)"))
                    .from(Alias::new("plats"))
                    .and_where(Expr::cust(format!("type_plat IN ({})", type_filter)))
                    .order_by(Alias::new("titre"), sea_orm::sea_query::Order::Asc)
                    .to_owned()
            };

            for (type_filter, cours_key, cours_label, field_name) in [
                ("'entree'",                               "entree",  "Entrées",  "entrees"),
                ("'specialite','viande','poisson','plat'", "plat",    "Plats",    "plats_menu"),
                ("'dessert'",                              "dessert", "Desserts", "desserts"),
            ] {
                let choices: Vec<(String, String)> = db.query_all(&plat_rows_for(type_filter)).await
                    .unwrap_or_default().iter()
                    .filter_map(|r| {
                        let id = r.try_get_by_index::<String>(0).ok()?;
                        let label = r.try_get_by_index::<String>(1).ok()?;
                        Some((id, label))
                    }).collect();
                let selected = if let Some(ref oid) = object_id {
                    let stmt = Query::select()
                        .expr(Expr::cust("CAST(plat_id AS TEXT)"))
                        .from(Alias::new("menu_resto_plat"))
                        .and_where(Expr::cust(format!("CAST(menu_id AS TEXT) = '{}'", oid)))
                        .and_where(Expr::cust(format!("cours = '{}'", cours_key)))
                        .to_owned();
                    db.query_all(&stmt).await.unwrap_or_default().iter()
                        .filter_map(|r| r.try_get_by_index::<String>(0).ok())
                        .collect()
                } else { vec![] };
                fields.push(M2mFieldOptions {
                    field_name: field_name.to_string(),
                    label: cours_label.to_string(),
                    choices,
                    selected,
                });
            }
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("titre", "Titre"), ("prix", "Prix"), ("disponible", "Disponible")]));
    registry.register(
        ResourceEntry::new(meta, form_builder)
            .with_list_fn(list_fn)
            .with_get_fn(get_fn)
    ...
    let form_builder: FormBuilder = Arc::new(|_db: ADb, _vec: Vec<std::string::String>, data: StrMap, tera: ATera, csrf: String, method: Method| {
        Box::pin(async move {
            let form = menu_resto_plat::AdminForm::build_with_data(&data, tera, &csrf, method).await;
    ...
    let count_fn: CountFn = Arc::new(|db: ADb, _search: Option<String>| {
        Box::pin(async move {
            menu_resto_plat::Entity::find().count(&*db).await
        })
    });
    ...
    let delete_fn: DeleteFn = Arc::new(|db: ADb, id: String| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            menu_resto_plat::Entity::delete_by_id(id).exec(&*db).await.map(|_| ())
        })
    });
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            menu_resto_plat::admin_from_form(&data, Some(id.into()))
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            menu_resto_plat::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_stmt_cours = Query::select().expr(Expr::cust("COUNT(DISTINCT cours)")).from(Alias::new(menu_resto_plat::Entity.table_name())).and_where(Expr::col(Alias::new("cours")).is_not_null()).to_owned();
            let count_row_cours = match db.query_one(&count_stmt_cours).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus_resto_composition.cours`: column not found in DB — {}", e); None } };
            let total_cours = count_row_cours.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_cours = Query::select().distinct().expr(Expr::cust("CAST(cours AS TEXT)")).from(Alias::new(menu_resto_plat::Entity.table_name())).and_where(Expr::col(Alias::new("cours")).is_not_null()).limit(page_size_cours).offset(cur_page_cours * page_size_cours).to_owned();
            let rows_cours = match db.query_all(&stmt_cours).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `menus_resto_composition.cours`: column not found in DB — {}", e); vec![] } };
            let mut vals_cours: Vec<String> = rows_cours.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_cours.sort_by(|a, b| a.cmp(b));
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "titre", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "type_boisson", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "prix", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "disponible", escaped)));
                query = query.filter(search_cond);
            }
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            boisson::admin_from_form(&data, Some(id.into()))
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            boisson::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "numero", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "type_commande", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "statut", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "mode_paiement", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "prix_total", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "created_at", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
                ...
                if !fk_ids.is_empty() {
                    ...
                    for row in &mut rows {
                        if let Some(key) = row.get("user_id").and_then(|v| v.as_i64().map(|n| n.to_string()).or_else(|| v.as_str().map(str::to_string))) {
                            if let Some(label) = label_map_user_id.get(&key) {
    ...
    let meta = meta.display(DisplayConfig::new().columns_include(vec![("numero", "N°"), ("user_id", "Client"), ("type_commande", "Type"), ("statut", "Statut"), ("mode_paiement", "Paiement"), ("prix_total", "Total"), ("created_at", "Date")]).list_filter(vec![("type_commande", "Type", 10u64), ("statut", "Statut", 10u64), ("mode_paiement", "Paiement", 10u64), ("avec_livraison", "Livraison", 10u64)]));
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            use sea_orm::{ConnectionTrait, ExprTrait};
            use sea_orm::sea_query::{Query, Alias, Expr};
            ...
            let count_stmt_type_commande = Query::select().expr(Expr::cust("COUNT(DISTINCT type_commande)")).from(Alias::new(commande::Entity.table_name())).and_where(Expr::col(Alias::new("type_commande")).is_not_null()).to_owned();
            let count_row_type_commande = match db.query_one(&count_stmt_type_commande).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `commandes.type_commande`: column not found in DB — {}", e); None } };
            let total_type_commande = count_row_type_commande.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_type_commande = Query::select().distinct().expr(Expr::cust("CAST(type_commande AS TEXT)")).from(Alias::new(commande::Entity.table_name())).and_where(Expr::col(Alias::new("type_commande")).is_not_null()).limit(page_size_type_commande).offset(cur_page_type_commande * page_size_type_commande).to_owned();
            let rows_type_commande = match db.query_all(&stmt_type_commande).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `commandes.type_commande`: column not found in DB — {}", e); vec![] } };
            let mut vals_type_commande: Vec<String> = rows_type_commande.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_type_commande.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
            result.insert("type_commande".to_string(), (vals_type_commande, total_type_commande));
            let page_size_statut = 10u64;
            let cur_page_statut = pages.get("statut").copied().unwrap_or(0);
            ...
            let count_stmt_avec_livraison = Query::select().expr(Expr::cust("COUNT(DISTINCT avec_livraison)")).from(Alias::new(commande::Entity.table_name())).and_where(Expr::col(Alias::new("avec_livraison")).is_not_null()).to_owned();
            let count_row_avec_livraison = match db.query_one(&count_stmt_avec_livraison).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `commandes.avec_livraison`: column not found in DB — {}", e); None } };
            let total_avec_livraison = count_row_avec_livraison.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            let stmt_avec_livraison = Query::select().distinct().expr(Expr::cust("CAST(avec_livraison AS TEXT)")).from(Alias::new(commande::Entity.table_name())).and_where(Expr::col(Alias::new("avec_livraison")).is_not_null()).limit(page_size_avec_livraison).offset(cur_page_avec_livraison * page_size_avec_livraison).to_owned();
            let rows_avec_livraison = match db.query_all(&stmt_avec_livraison).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `commandes.avec_livraison`: column not found in DB — {}", e); vec![] } };
            let mut vals_avec_livraison: Vec<String> = rows_avec_livraison.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_avec_livraison.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
            result.insert("avec_livraison".to_string(), (vals_avec_livraison, total_avec_livraison));
            Ok(result)
        })
    });
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "commande_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "note", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "statut", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "created_at", escaped)));
                query = query.filter(search_cond);
            }
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "nom", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "adresse", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "telephone", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "email", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            info_resto::admin_from_form(&data, Some(id.into()))
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string().to_string()))?;
            info_resto::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });

    let meta = meta.display(DisplayConfig::new().columns_include(vec![("nom", "Nom"), ("adresse", "Adresse"), ("telephone", "Téléphone"), ("email", "Email")]));
    registry.register(
        ResourceEntry::new(meta, form_builder)
            .with_list_fn(list_fn)
            .with_get_fn(get_fn)
    ...
    let form_builder: FormBuilder = Arc::new(|_db: ADb, _vec: Vec<std::string::String>, data: StrMap, tera: ATera, csrf: String, method: Method| {
        Box::pin(async move {
            let form = supplement::AdminForm::build_with_data(&data, tera, &csrf, method).await;
    ...
    let update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            supplement::admin_from_form(&data, Some(id.into()))
                .update(&*db).await.map(|_| ())
        })
    });
    ...
    let partial_update_fn: UpdateFn = Arc::new(|db: ADb, id: String, data: StrMap| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("invalid id".to_string()))?;
            supplement::admin_partial_update(&data, id.into())
                .update(&*db).await.map(|_| ())
        })
    });

    let meta = meta.display(DisplayConfig::new().columns_include(vec![("libelle", "Libellé"), ("prix", "Prix"), ("disponible", "Disponible")]));
    registry.register(
        ResourceEntry::new(meta, form_builder)
            .with_list_fn(list_fn)
            .with_get_fn(get_fn)
