...
pub async fn handle_avis(request: Request) -> AppResult<Response> {
    ...
    if !matches!(cmd.statut, StatutCommande::Termine | StatutCommande::Livre) {
        request
            .notices
            .error("Un avis ne peut être laissé que pour une commande terminée.".to_string())
            .await;
        return Ok(Redirect::to("/compte").into_response());
    }
    ...
    if existing.is_some() {
        request
            .notices
            .error("Vous avez déjà laissé un avis pour cette commande.".to_string())
            .await;
        return Ok(Redirect::to("/compte").into_response());
    }
    ...
    if !form.is_valid().await {
        request
            .notices
            .error("Formulaire invalide.".to_string())
            .await;
        return Ok(Redirect::to("/compte").into_response());
    }
...
pub async fn handle_avis_supprimer(request: Request) -> AppResult<Response> {
    ...
    if deleted > 0 {
        request
            .notices
            .success("Votre avis a été supprimé.".to_string())
            .await;
    }
