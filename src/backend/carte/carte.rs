use crate::backend::carte::{CarteGarniture, CarteMenuResto, CartePage, CartePlat, CoursMenu, build_boissons, build_supplements};
use crate::entities::{allergene, garniture, menu_resto, menu_resto_plat, plat, plat::TypePlat, plat_allergene};
use runique::prelude::*;
use std::collections::HashMap;

pub async fn get_carte(db: &sea_orm::DatabaseConnection) -> CartePage {
    let plat_models: Vec<plat::Model> = search!(plat::Entity => asc TypePlat, asc Titre,)
        .all(db)
        .await
        .unwrap_or_default();
    ...
    let plat_ids: Vec<i32> = plat_models.iter().map(|p| p.id).collect();
    ...
    for p in plat_models {
        ...
        let carte_plat = CartePlat {
            ...
            prix_supplement_legumes: format!("{:.2}", p.prix_supplement_legumes.unwrap_or_default()),
    ...
    CartePage { entrees, specialites, viandes, poissons, desserts, menus, supplements, boissons }
}
...
async fn build_menus(db: &sea_orm::DatabaseConnection) -> Vec<CarteMenuResto> {
    let menus = search!(menu_resto::Entity => asc Titre,)
    ...
    let plats_map: HashMap<i32, plat::Model> = search!(plat::Entity => Id in (plat_ids),)
        .all(db)
        .await
        .unwrap_or_default()
        .into_iter()
    ...
    for link in allergene_links {
        if let Some(label) = allergene_labels.get(&link.allergene_id) {
            allergenes_by_plat.entry(link.plat_id).or_default().push(label.clone());
        }
    }
    ...
    menus
        ...
        .map(|m| {
            ...
            for lien in liens.iter().filter(|l| l.menu_id == m.id) {
                if let Some(p) = plats_map.get(&lien.plat_id) {
                    let garnitures = match p.type_plat {
                        TypePlat::Entree | TypePlat::Dessert => vec![],
                        _ => feculents.clone(),
                    };
            ...
            let cours = [
                CoursMenu { label: "Entrée".to_string(),  key: "entree".to_string(),  plats: entrees },
                CoursMenu { label: "Plat".to_string(),    key: "plat".to_string(),    plats },
                CoursMenu { label: "Dessert".to_string(), key: "dessert".to_string(), plats: desserts },
            ]
            .into_iter()
